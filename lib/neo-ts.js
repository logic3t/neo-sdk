...
(function (Neo) {
    ...
    var BigInteger = (function () {
        ...
        BigInteger.random = function (bitLength) {
            if (bitLength == 0)
                return BigInteger.Zero;
...
(function (ThinNeo) {
    ...
    var Helper = (function () {
        ...
        Helper.Sign = function (message, privateKey) {
            let PublicKey = Neo.Cryptography.ECPoint.multiply(Neo.Cryptography.ECCurve.secp256r1.G, privateKey);
            let key = new Neo.Cryptography.ECDsaCryptoKey(PublicKey, privateKey);
            let ecdsa = new Neo.Cryptography.ECDsa(key);
            let sign = new Uint8Array(ecdsa.sign(message));
            return sign;
        };
...
(function (ThinNeo) {
    ...
    var Transaction = (function () {
        ...
        Transaction.prototype.AddWitness = function (signdata, pubkey, addrs = null) {
            if(addrs){
                let msg = this.GetMessage();
                let bsign = ThinNeo.Helper.VerifySignature(msg, signdata, pubkey);
                if (bsign == false)
            ...
            let vscript = ThinNeo.Helper.GetAddressCheckScriptFromPublicKey(pubkey);
            let sb = new ThinNeo.ScriptBuilder();
            sb.EmitPushBytes(signdata);
            let iscript = sb.ToArray();
    ...
    (function (Cryptography) {
        var RandomNumberGenerator = (function () {
            ...
            RandomNumberGenerator.getRandomValues = function (array) {
                var data = new Float64Array([Date.now()]);
                var key = new Uint8Array(Cryptography.Sha256.computeHash(data));
                var aes = new Cryptography.Aes(key, RandomNumberGenerator.getWeakRandomValues(16));
...
(function (Neo) {
    ...
    (function (Cryptography) {
        ...
        if (global["crypto"] == null)
            global.crypto = { subtle: null, getRandomValues: null };
        ...
        function hook_ripemd160() {
            var digest_old = global["crypto"].subtle.digest;
            global["crypto"].subtle.digest = function (algorithm, data) {
        ...
        hook_ripemd160();
        function hook_ecdsa() {
            var exportKey_old = global["crypto"].subtle.exportKey;
            global["crypto"].subtle.exportKey = function (format, key) {
        ...
        try {
            global["crypto"].subtle.generateKey({ name: "ECDSA", namedCurve: "P-256" }, false, ["sign", "verify"]).catch(hook_ecdsa);
        }
        catch (ex) {
            hook_ecdsa();
        }
